...
class InSampleAC(base.Agent):
    def __init__(
        ...
        split_vals = exp_path.split("/")
        env_name=split_vals[-4]
        dataset=split_vals[-3]
    ...
    def compute_loss_pi(self, data):
        ...
        clipped = torch.clip(
            torch.exp((min_Q - value) / self.tau - beh_log_prob),
            self.eps,
            self.exp_threshold,
        )
    ...
    def sync_target(self):
        with torch.no_grad():
            for p, p_targ in zip(
                self.ac.q1q2.parameters(), self.ac_targ.q1q2.parameters()
            ):
