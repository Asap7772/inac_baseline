...
class Replay:
    ...
    def sample(self, batch_size=None):
        ...
        sampled_indices = [
            self.rng.randint(0, len(self.data)) for _ in range(batch_size)
        ]
...
class Agent:
    def __init__(
        ...
        self.populate_states, self.populate_actions, self.populate_true_qs = (
            None,
            None,
            None,
        )
    ...
    def get_data(self):
        ...
        data = {"obs": in_, "act": actions, "reward": r, "obs2": ns, "done": t}
        return data
    ...
    def fill_offline_data_to_buffer(self):
        ...
        for idx in range(len(train_s)):
            self.replay.feed(
                [train_s[idx], train_a[idx], train_r[idx], train_ns[idx], train_t[idx]]
            )
    ...
    def log_file(self, elapsed_time=-1, test=True):
        mean, median, min_, max_ = self.log_return(
            self.ep_returns_queue_train, "TRAIN", elapsed_time
        )
        if test:
            (
                self.populate_states,
                self.populate_actions,
                self.populate_true_qs,
            ) = self.populate_returns(log_traj=True)
            self.populate_latest = True
            mean, median, min_, max_ = self.log_return(
                self.ep_returns_queue_test, "TEST", elapsed_time
            )
            try:
                normalized = np.array(
                    [
                        self.eval_env.env.unwrapped.get_normalized_score(ret_)
                        for ret_ in self.ep_returns_queue_test
                    ]
                )
                mean, median, min_, max_ = self.log_return(
                    normalized, "Normalized", elapsed_time
                )
            except:
                pass
        return mean, median, min_, max_
